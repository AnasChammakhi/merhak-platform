...
function Orders() {
  ...
  const [showPaid, setShowPaid] = useState(() => {
    try { const s = localStorage.getItem("ordersFilters"); if (s) return JSON.parse(s).showPaid ?? true; } catch (e) {} return true;
  });
  const [showDeposit, setShowDeposit] = useState(() => {
    try { const s = localStorage.getItem("ordersFilters"); if (s) return JSON.parse(s).showDeposit ?? true; } catch (e) {} return true;
  });
  const [showUnpaid, setShowUnpaid] = useState(() => {
    try { const s = localStorage.getItem("ordersFilters"); if (s) return JSON.parse(s).showUnpaid ?? true; } catch (e) {} return true;
  });

  useEffect(() => {
    try {
      localStorage.setItem("ordersFilters", JSON.stringify({ showPaid, showDeposit, showUnpaid }));
    } catch (e) {}
  ...
  const filteredOrders = useMemo(() => {
    ...
    return result;
  }, [orders, search, typeFilter, statusFilter, showPaid, showDeposit, showUnpaid]);
    ...
    <div className="p-8 lg:p-10">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-semibold text-[#10212f]">Gestion des commandes</h1>
          <p className="mt-2 text-sm text-[#667785]">Gérez toutes les commandes standard et sur-mesure.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-[#e5f1f8] bg-[#f7fbfe]">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-[#10212f]">Type</th>
            ...
            <tbody className="divide-y divide-[#edf4f8]">
              {loading ? (
                <tr>
                  <td colSpan="8" className="px-6 py-16 text-center text-[#667785]">Chargement...</td>
                </tr>
              ) : filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan="8" className="px-6 py-16 text-center text-[#667785]">Aucune commande trouvée.</td>
                </tr>
              ) : (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="transition hover:bg-[#fafdff]">
                    ...
                    <td className="px-6 py-5">
                      <span className={`inline-block rounded-full px-2.5 py-1 text-xs font-medium ${order.type === 'CUSTOM' ? 'bg-purple-100 text-purple-700' : 'bg-blue-50 text-blue-700'}`}>
                        {order.type === 'CUSTOM' ? 'Sur-mesure' : 'Standard'}
                      </span>
                    </td>
                    <td className="px-6 py-5">
