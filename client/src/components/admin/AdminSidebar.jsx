...
function AdminSidebar() {
  ...
  const menuClass = ({ isActive }) =>
    `flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition ${
      isActive ? "bg-white text-[#0f73c4]" : "text-white/70 hover:bg-white/10 hover:text-white"
    }`;
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[#0f73c4]/60" />

      <div className="relative z-10 flex h-full flex-col">
        <div className="border-b border-white/15 px-7 py-7">
          <img src={logoWhite} alt="MERHAK" className="h-10 w-auto" />
        ...
        <nav className="flex-1 space-y-2 p-4">
          ...
          <NavLink to="/admin/clients" className={menuClass}>
          ...
          <NavLink to="/admin/custom-orders" className={menuClass}>
            <ScissorsIcon className="h-5 w-5" />
            Sur-Mesure
          </NavLink>

          <NavLink to="/admin/calendar" className={menuClass}>
